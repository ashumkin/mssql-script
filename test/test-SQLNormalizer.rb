...
require 'test/unit'
require 'stringio'
require File.expand_path('../../SQLNormalizer', __FILE__)
require 'rake'
require 'unixdiff'
...
class TestSQLNormalizer < Test::Unit::TestCase
private
  ...
  def _test_files(mask)
    ...
    FileList[File.expand_path('../resources/source/' + mask, __FILE__)].each do |f|
      ...
      test_text_output = SQL::FileReader.readlines(f).map!{ |line| line.chomp }
      test_text_expected = SQL::FileReader.readlines(f_expected).map!{ |line| line.chomp }
...
public
  def setup
    ...
    @opts = SQLNormalizerOptions.new(args)
    @sqlNormalizer = SQLNormalizer.new(@opts)
  end
